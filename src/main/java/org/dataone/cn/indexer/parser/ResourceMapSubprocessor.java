...
public class ResourceMapSubprocessor implements IDocumentSubprocessor {
    ...
    public Map<String, SolrDoc> processDocument(String identifier, Map<String, SolrDoc> docs,
        ...
        try {
            ...
            perfLog.info(String.format("%-60s, %20d", "ResourceMapSubprocessor.processDocument() XmlDocumentUtility.generateXmlDocument()", System.currentTimeMillis() - fetchXmlStart));
            
            long procResMapStart = System.currentTimeMillis();
            processedDocs = processResourceMap(resourceMapDoc, doc);
            perfLog.info(String.format("%-60s, %20d", "ResourceMapSubprocessor.processResourceMap()", System.currentTimeMillis() - procResMapStart));
    ...
    private List<SolrDoc> processResourceMap(SolrDoc indexDocument, Document resourceMapDocument)
        ...
        perfLog.info(String.format("%-60s, %20d", "ResourceMapFactory.buildResourceMap() create ResourceMap from Document", System.currentTimeMillis() - buildResMapStart));
        ...
        perfLog.info(String.format("%-60s, %20d", "ResourceMap.getAllDocumentIDs() referenced in ResourceMap", System.currentTimeMillis() - getReferencedStart));
        
        long clearSidChainStart = System.currentTimeMillis();
        this.clearSidChain(indexDocument.getIdentifier(), documentIds);
        perfLog.info(String.format("%-60s, %20d", "ResourceMapSubprocessor.clearSidChain() removing obsoletes chain from Solr index", System.currentTimeMillis() - clearSidChainStart));
        ...
        perfLog.info(String.format("%-60s, %20d", "HttpService.getDocumentsById() get existing referenced ids' Solr docs", System.currentTimeMillis() - getSolrDocsStart));
