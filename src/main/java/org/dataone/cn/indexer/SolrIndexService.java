...
public class SolrIndexService {
    ...
    public SolrElementAdd processObject(String id, InputStream systemMetaDataStream,
        ...
        try {
            ...
            perfLog.info(String.format("%-60s, %20d", systemMetadataProcessor.getClass().getSimpleName() + ".processDocument() processing sysmeta", System.currentTimeMillis() - sysmetaProcStart));
        } catch (Exception e) {
            ...
            if (subprocessor.canProcess(formatId)) {
                try {
                    ...
                    if (!objectStream.getFD().valid()) {
                        ...
                        perfLog.info(String.format("%-60s, %20d", "SolrIndexService.processObject() " + subprocessor.getClass().getSimpleName() + ".processDocument() total subprocessor processing time for format: " + formatId + "", System.currentTimeMillis() - scimetaProcStart));
                    }
                } catch (Exception e) {
        ...
        perfLog.info(String.format("%-60s, %20d", "SolrIndexService.processObject() merging docs", System.currentTimeMillis() - mergeProcStart));
        ...
        perfLog.info(String.format("%-60s, %20d", "SolrIndexService.processObject() total processing time for id " + id, System.currentTimeMillis() - processObjStart));
        return addCommand;
    }
    ...
    public void insertIntoIndex(String id, InputStream systemMetaDataStream, String objectPath)
        ...
        sendCommand(addCommand);
        perfLog.info(String.format("%-60s, %20d", "SolrIndexService.sendCommand(SolrElementAdd) adding docs into Solr index", System.currentTimeMillis() - solrAddStart));
    }
