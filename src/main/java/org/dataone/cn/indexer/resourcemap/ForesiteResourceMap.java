...
public class ForesiteResourceMap implements ResourceMap 
{
	/* Class contants */
	 ...
	 private static final String RESOURCE_MAP_FORMAT = 
	...
	private SolrDoc _mergeMappedReference(ResourceEntry resourceEntry, 
			SolrDoc mergeDocument) 
	{
	...
	private void _init(InputStream is)
			throws OREException, URISyntaxException, UnsupportedEncodingException, 
				   OREParserException 		   
		
		{
			/* Creates the indentifier map from the doc */
			Map<Identifier, Map<Identifier, List<Identifier>>> tmpResourceMap = 
			...
			for(Map.Entry<Identifier, List<Identifier>> entry : 
				identiferMap.entrySet())
			{
				ForesiteResourceEntry documentsResourceEntry = resourceMap.get(entry.getKey().getValue());
				...
				for(Identifier documentedByIdentifier : entry.getValue())
				{
					...
					ForesiteResourceEntry documentedByResourceEntry = 
							resourceMap.get(documentedByIdentifier.getValue());
					
					if(documentedByResourceEntry == null)
					{
						documentedByResourceEntry = 
							new ForesiteResourceEntry(documentedByIdentifier.getValue(), this);
						
						resourceMap.put(documentedByIdentifier.getValue(), 
								documentedByResourceEntry);
					}
					
					documentedByResourceEntry.addDocumentedBy(
							entry.getKey().getValue());	
				}
			}
		}
	
	public ForesiteResourceMap(String doc) 
	...
	public ForesiteResourceMap(InputStream is) 
		throws UnsupportedEncodingException, OREException, URISyntaxException, 
		       OREParserException 
	{
		_init(is);
	}
	
	public ForesiteResourceMap(Document doc) 
		throws DOMException, LSException, ClassCastException, OREException, 
	...
	public Set<ResourceEntry> getMappedReferences() 
	{
		...
		for(ResourceEntry resourceEntry : this.resourceMap.values())
		{
            ...
            {
                if(resourceEntry.getIdentifier().equals(
